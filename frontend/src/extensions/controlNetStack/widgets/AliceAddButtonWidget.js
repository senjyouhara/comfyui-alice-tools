...
export class AliceAddButtonWidget extends AliceBaseWidget {
  constructor() {
    super("__alice_controlnet_add_button__", 30);
  ...
  draw(ctx, node, width, y) {
    ...
    ctx.fillText("+ Add ControlNet", buttonX + buttonWidth / 2, y + drawHeight / 2);
    ctx.restore();
    ...
    this.hitAreas.add = {
      ...
      onClick: (event, _pos, currentNode) => {
        ...
        showControlNetChooser(event, (value) => {
          if (value && value !== "None") {
            currentNode._aliceAddRow({ ...DEFAULT_ROW_VALUE, controlnet: value });
          }
        });
      },
    };
